...
public class AppleIdentityProvider : IExternalIdentityProvider
{
    ...
    public AppleIdentityProvider(IHttpClientFactory httpClientFactory, IJwksStore jwksStore)
    {
        ...
        string authorizationCode, string redirectUri, string clientId, string clientSecret, string? scopes, string? codeVerifier = null, string? nonce = null, CancellationToken ct = default)
    {
        try
        {
            ...
            if (!tokenResponse.IsSuccessStatusCode)
                return Result<ExternalIdentityClaims>.Failure("PROVIDER_ERROR", "Error al intercambiar código por token en Apple");

            var tokenData = await tokenResponse.Content.ReadFromJsonAsync<AppleTokenResponse>(ct);
            if (tokenData?.IdToken == null)
                return Result<ExternalIdentityClaims>.Failure("PROVIDER_ERROR", "No se recibió id_token de Apple");
            ...
            if (string.IsNullOrEmpty(name))
                name = jwt.Claims.FirstOrDefault(c => c.Type == "given_name")?.Value;
        ...
        {
            return Result<ExternalIdentityClaims>.Failure("TOKEN_EXPIRED", "El id_token de Apple ha expirado");
        }
        ...
        {
            return Result<ExternalIdentityClaims>.Failure("SIGNATURE_INVALID", "La firma del id_token de Apple no es válida");
        }
        catch (Exception ex)
        {
            return Result<ExternalIdentityClaims>.Failure("PROVIDER_ERROR", $"Error de comunicación con Apple: {ex.Message}");
        }
    }
    ...
    public Task<Result<TokenRefreshResult>> RefreshTokenAsync(string refreshToken, string clientId, string clientSecret, string? scopes = null, CancellationToken ct = default)
        => Task.FromResult(Result<TokenRefreshResult>.Failure("NOT_SUPPORTED", "Apple no soporta renovación de tokens"));
    ...
    {
        var scope = Uri.EscapeDataString(scopes ?? "name email");
        ...
        var url = $"https://appleid.apple.com/auth/authorize?client_id={Uri.EscapeDataString(clientId)}&response_type=code%20id_token&redirect_uri={redirect}&scope={scope}&response_mode=form_post{s}{pkce}{n}";
